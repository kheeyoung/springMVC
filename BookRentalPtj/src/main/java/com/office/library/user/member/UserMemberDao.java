...
public class UserMemberDao {
	...
	public boolean isUserMember(String u_m_id) {
		...
		String sql =  "SELECT COUNT(*) FROM tbl_user_member WHERE u_m_id = ?"; //쿼리문 작성(아이디 중복 확인 용)
	...
	public int insertUserAccount(UserMemberVo userMemberVo) {
		...
		+"u_m_pw, u_m_name, u_m_gender, u_m_mail, u_m_phone, u_m_reg_date, u_m_mod_date) "
		+ "VALUES(?, ?, ?, ?, ?, ?, NOW(), NOW())";
		...
		try {
			result = jdbcTemplate.update(sql, userMemberVo.getU_m_id(),
												...
												userMemberVo.getU_m_name(),userMemberVo.u_m_gender,
												userMemberVo.getU_m_mail(),userMemberVo.getU_m_mail());
	...
	public UserMemberVo selectUser(UserMemberVo userMemberVo) { 
		...
		try {
			userMemberVos = jdbcTemplate.query(sql, new RowMapper<UserMemberVo>() {
				@Override
				public UserMemberVo mapRow (ResultSet rs, int rowNum) throws SQLException{ //MAPROW=RowMapper의 추상클래스 구현. 각 데이터를 rs로 받아 매핑한다.
		...
		try {
			userMemberVos=jdbcTemplate.query(sql, new RowMapper<UserMemberVo>() {
				...
				public UserMemberVo mapRow(ResultSet rs, int rowNum) throws SQLException{
					UserMemberVo userMemberVos = new UserMemberVo();
					
					userMemberVos.setU_m_no(rs.getInt("u_m_no"));
					userMemberVos.setU_m_id(rs.getString("u_m_id"));
					userMemberVos.setU_m_pw(rs.getString("u_m_pw"));
					userMemberVos.setU_m_name(rs.getString("u_m_name"));
					userMemberVos.setU_m_gender(rs.getString("u_m_gender"));
					userMemberVos.setU_m_mail(rs.getString("u_m_mail"));
					userMemberVos.setU_m_phone(rs.getString("u_m_phone"));
					userMemberVos.setU_m_reg_date(rs.getString("u_m_reg_date"));
					userMemberVos.setU_m_mod_date(rs.getString("u_m_mod_date"));
