...
public class BookDao {
	...
	public int insertBook(BookVo bookVo) {
		...
		String sql="INSERT INTO tbl_book(b_thumbnail,"
				+ "b_name, b_author, b_publisher, b_publish_year, b_isbn, b_call_number, b_rantal_able, b_reg_date, b_mod_date) "
				+ "VALUES (?,?,?,?,?,?,?,?,NOW(),NOW())";
		...
		try {
			result=jdbcTemplate.update(sql, bookVo.getB_thumbnail(),bookVo.getB_name(),bookVo.getB_author(),
					bookVo.getB_publisher(),bookVo.getB_publish_year(),bookVo.getB_isbn(),bookVo.getB_call_number(),bookVo.getB_rantal_able());
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	public List<BookVo> selectBooksBySearch(BookVo bookVo) {
		...
		try {
			bookVos=jdbcTemplate.query(sql,new RowMapper<BookVo>() {
				@Override
				public BookVo mapRow (ResultSet rs, int rowNum) throws SQLException{ //MAPROW=RowMapper의 추상클래스 구현. bookVo의 각 데이터를 rs로 받아 매핑한다.
	...
	public BookVo selectBook(int b_no) {
		...
		return bookVos.size()>0 ? bookVos.get(0) : null; //위의 전체 검색 기능(selectBooksBySearch)의 경우 여러개가 나올 수 있어서 전체 배열(검색 해서 나온 Vo)을 세보았지만, 여기서는 1개라서 첫번째 것만 본다.
	}
	...
	public int updateBook(BookVo bookVo) {
		...
		if(bookVo.getB_thumbnail() != null) {//이미지가 바뀐 경우
			sql+="b_thumbnail = ?, ";
			args.add(bookVo.getB_thumbnail());
		}
		
		//각 정보 받아오기 (sql문 완성하기 위함)
		sql+="b_name = ?, ";
	...
	public List<RentalBookVo> selectRentalBooks() {
		...
		String sql =  "SELECT * FROM tbl_rental_book rb "
					...
					+ "WHERE rb.rb_end_date = '1000-01-01' ORDER BY um.u_m_id ASC, rb.rb_reg_date DESC"; //각 테이블 조인하여 정보 조회
	...
	public int updateRentalBooks(int rb_no) {
		System.out.println("[BookDao] updateRentalBooks()");
	...
	public List<HopeBookVo> getHopeBooks() {
		System.out.println("[BookDao] getHopeBooks()");
		
		String sql =  "SELECT * FROM tbl_hope_book hb "
					+ "JOIN tbl_user_member um "
					+ "ON hb.u_m_no = um.u_m_no "
					+ "ORDER BY hb.hb_no DESC"; //각 테이블 조인하여 정보 조회
...
System.out.println("[BookDao] selectAllBooks()");
		...
		String sql =  "SELECT * FROM tbl_book b ORDER BY b_reg_date DESC"; 
